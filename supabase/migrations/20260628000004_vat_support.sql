-- ============================================================
-- LenaBeauty — VAT / Tax support (GCC compliance)
--
-- Adds a `tax` column to invoices and updates process_checkout_v1 to
-- compute VAT from center_settings.tax_rate and include it in the total.
--
-- Pricing model (must match src/pages/PosInvoicesPage.tsx):
--   net   = max(0, subtotal - discount - loyaltyDiscount)
--   tax   = round(net * tax_rate / 100, 3)
--   total = net + tax
--   earnedPoints = floor(net)   -- loyalty earned on the pre-tax net
--
-- HOW TO RUN: Supabase SQL Editor, AFTER 20260628000003_checkout_rpc.sql.
-- ============================================================

-- 1. Add the tax column (idempotent).
ALTER TABLE public.invoices
  ADD COLUMN IF NOT EXISTS tax NUMERIC(12,3) NOT NULL DEFAULT 0;

-- 2. Recreate the checkout RPC with VAT handling.
...
BEGIN
    ...
    LOOP
        ...
        v_net := v_net + (v_item_price * v_item_qty);
    END LOOP;

    -- Loyalty: 1 point = 1 OMR, capped at (subtotal - manual discount).
    IF COALESCE(p_use_loyalty_points, false) THEN
        SELECT LEAST(GREATEST(v_net - v_discount, 0.000), COALESCE(c.loyalty_points, 0)::NUMERIC)
    ...
    -- Net after all discounts (pre-tax).
    v_net := GREATEST(0.000, v_net - v_discount - v_loyalty_discount);

    -- VAT from center settings.
    ...
    v_total := v_net + v_tax;
    v_earned_points := FLOOR(v_net); -- earn on pre-tax net
    ...
    VALUES (
        ...
        p_payment_method, v_total, v_discount, v_tax, v_loyalty_discount::INTEGER
    )
