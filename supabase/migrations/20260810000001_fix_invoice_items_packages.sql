-- ============================================================================
-- Fix invoice_items integrity for package sales
-- ----------------------------------------------------------------------------
-- Root cause (Sales/Reports data layer):
--   1. process_checkout_v1 expanded a "package" line into one invoice_item per
--      included service with price = 0, so itemized views (invoice print,
--      sales report, dashboard) showed zero-priced rows whose sum never matched
--      the invoice total.
--   2. invoice_items had no package reference at all, so reports could not
--      distinguish a package sale from a free service, and the invoice print
--      fallback mislabeled it.
--
-- Fix:
--   * Add invoice_items.package_id (FK -> service_packages, nullable so the
--     schema stays backward compatible with pre-existing service/product rows).
--   * Rewrite process_checkout_v1 so a package line persists as ONE invoice_item
--     row carrying the actual package price and quantity, exactly like service
--     and product lines. Historical rows (price = 0 from old package expansion)
--     remain untouched and are handled defensively by the report mappers.
-- ============================================================================

ALTER TABLE public.invoice_items
  ADD COLUMN IF NOT EXISTS package_id UUID REFERENCES public.service_packages(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS idx_invoice_items_package ON public.invoice_items(package_id);

-- ----------------------------------------------------------------------------
-- Checkout RPC (replaces the package-expansion implementation)
-- ----------------------------------------------------------------------------
...
BEGIN
    ...
    LOOP
        ...
        v_item_qty   := NULLIF(v_item->>'qty', '')::NUMERIC;
        v_item_price := NULLIF(v_item->>'price', '')::NUMERIC;
        -- Reset per line so a package line cannot leak a previous service id.
        v_service_id := NULL;
        v_product_id := NULL;
        v_package_id := NULL;
        ...
        -- Every line — service, product, OR package — persists as exactly one
        -- invoice_item with its real price, so itemized sums match the invoice
        -- total and reports never show zero-priced rows.
        INSERT INTO public.invoice_items (invoice_id, service_id, product_id, package_id, price, quantity)
        VALUES (v_invoice_id, v_service_id, v_product_id, v_package_id, v_item_price, v_item_qty::INTEGER);
