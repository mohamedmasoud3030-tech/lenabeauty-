...
export function PageHeader({ icon, title, subtitle, actions, className }: PageHeaderProps) {
  ...
  return (
    <div
      className={clsx(
        "flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-8",
        className,
      )}
    >
      <div className="flex items-center gap-3 sm:gap-6 min-w-0">
        <div className="h-14 w-14 sm:h-16 sm:w-16 rounded-[1.5rem] sm:rounded-[2rem] bg-primary/10 flex items-center justify-center text-primary shadow-inner shrink-0">
          {icon}
        </div>
        <div className="space-y-1 min-w-0">
          <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-foreground tracking-tight truncate">
            {title}
          </h1>
          {subtitle && (
