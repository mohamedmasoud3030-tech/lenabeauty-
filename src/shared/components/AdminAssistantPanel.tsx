...
const fieldClass = "min-h-11 w-full rounded-xl border border-border bg-background px-3 py-2 text-sm font-bold outline-none focus:border-primary focus:ring-2 focus:ring-primary/15";
...
export function AdminAssistantPanel({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <div
      ...
      className="fixed z-[var(--z-overlay)] inset-x-3 bottom-[calc(5.25rem+env(safe-area-inset-bottom,0px))] flex max-h-[min(28rem,68dvh)] flex-col overflow-hidden rounded-2xl border border-border bg-card shadow-2xl print:hidden lg:inset-x-auto lg:end-6 lg:bottom-24 lg:w-[22rem]"
    >
      <div className="flex shrink-0 items-center gap-2 border-b border-border px-3 py-2">
        <MessageCircle className="h-4 w-4 text-primary" aria-hidden="true" />
        <h2 id="admin-assistant-title" className="min-w-0 flex-1 truncate text-sm font-bold">{t("Admin assistant")}</h2>
        <button
          ref={closeRef}
          type="button"
          onClick={onClose}
      ...
      <p className="shrink-0 border-b border-border px-3 py-2 text-[11px] leading-relaxed text-muted-foreground">
      ...
      <div className="min-h-0 flex-1 overflow-auto p-3 space-y-3">
        {!active ? (
          <div className="space-y-3">
            <h3 className="text-sm font-bold">{t("Activate with a Gemini API key")}</h3>
            <p className="text-xs text-muted-foreground">{t("The key stays on this device. It is not saved on the server.")}</p>
          ...
          <>
            <div className="flex items-center justify-between gap-2">
              <p className="text-xs font-bold">{t("Assistant is on")}</p>
              <button type="button" onClick={removeKey} className="min-h-11 rounded-xl border border-border px-3 text-xs font-bold">
                {t("Remove key")}
              </button>
            </div>
            {history.length === 0 ? (
              <p className="text-xs text-muted-foreground">{t("Conversation is not saved.")}</p>
            ) : (
      ...
      {active ? (
        <form onSubmit={(event) => void send(event)} className="flex shrink-0 gap-2 border-t border-border p-3">
