...
The work is carried on the Arena session branch (`arena/019ffa56-lenabeauty`), which is
checked out at exactly the verified baseline SHA. No other branch was created or pushed.
...
- ❌ No Supabase migrations created or edited.
- ❌ No RLS policies, RPCs, triggers, tables, views, or remote data created/edited.
- ❌ Nothing applied to Demo, Staging, or Production.
...
- ❌ No generated Supabase types committed (there are none to commit — see `03_VERIFIED_DRIFT_REGISTER.md`).
...
```bash
npm run audit:replay   # PGlite replay + idempotency + catalog inventory
...
Machine-readable outputs are written to `docs/database-contract/artifacts/`:

- `replay-report.json` — per-migration replay + idempotency + translations
- `schema-inventory.json` — replayed catalog (tables/columns/enums/constraints/FKs/indexes/triggers/functions/policies/grants/RLS)
- `frontend-usage.json` — frontend tables/RPCs/embeds/storage
- `contract-matrix.json` — resolution results per table/RPC/embed
...
(PostgreSQL 18.3, WASM), after installing a **compatibility preamble** that only stubs the
Supabase-hosted surfaces the canonical SQL references (`auth.users`, `auth.uid()`, `anon`/
`authenticated` roles, `storage.buckets`, `storage.objects`). Two **documented, logged
...
2. The `appointments_no_scheduled_staff_overlap` `EXCLUDE USING gist` constraint (which
   requires btree_gist's gist `=` operator class) is replaced with a `RAISE NOTICE`
   surrogate inside its enclosing `DO` block; its canonical DDL is preserved verbatim in
   `schema-inventory.json` → `canonical_only`.

Every translation is recorded in `replay-report.json` → `translations`. No translation is
silent.
