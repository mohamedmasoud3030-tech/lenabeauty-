...
schema has received the complete visit/recipe migration set recorded below,
including the write-boundary and duplicate-line consumption hardening discovered
during hosted review.
...
- **Server-authoritative checkout**: `process_checkout_idempotent_v1` (SECURITY
  DEFINER) is the only client checkout authority for `invoices`, `payments`,
  `invoice_items`, recipe consumption and the visit→`COMPLETED` transition.
- **Recipe writes are RPC-only**: authenticated clients may read recipes but may
  not insert/update/delete `service_recipes` or `service_recipe_items` directly.
...
- `supabase/migrations/20260901100838_visit_lifecycle_recipes.sql`
  ...
  - `app_private.consume_invoice_recipes_v1` and the appointment-aware
    `process_checkout_idempotent_v1` foundation.
- `supabase/migrations/20260901101133_visit_recipe_index_hardening.sql`
  - Adds the six feature-scoped foreign-key access-path indexes identified by
    the hosted performance advisor.
- `supabase/migrations/20260901102643_recipe_write_boundary_hardening.sql`
  - Removes anonymous access to recipe tables.
  - Removes authenticated direct INSERT/UPDATE/DELETE access and leaves
    center-scoped SELECT only, keeping `save_service_recipe_v1` as the write
    authority.
  - Is replay-safe (`DROP POLICY IF EXISTS` before replacement policies).
- `supabase/migrations/20260901102758_recipe_consumption_aggregation_hardening.sql`
  - Aggregates repeated invoice lines by `service_id` before recipe consumption,
    preventing the `(invoice_id, service_id, product_id)` idempotency key from
    silently under-consuming later duplicate lines.
  - Keeps the internal consumer non-executable by PUBLIC/anon/authenticated.

The migration versions above intentionally match the canonical LENA Beauty Demo
migration history. The database-contract replay currently discovers **41
migrations: 40 automated + exactly one explicitly excluded manual bootstrap**.
The test contract no longer hard-codes that count: it asserts the single manual
exclusion and requires every automated migration to replay idempotently. Repeat
application produces an identical schema fingerprint with **0 high / 0 medium**
database-contract findings.
...
- **Ports/adapter**: new `ServiceRecipeRepository` (`getForService`,
  ...
  `save_service_recipe_v1`. Direct table writes are intentionally unavailable
  to authenticated clients.
- **Checkout consumption**: duplicate service invoice lines are aggregated before
  recipe consumption; retries remain idempotent and do not double-decrement
  stock.
...
final #52 closure. No tests, lint rules or canonical gates were weakened.

`src/__tests__/audit.replay.test.ts` now protects the current database behavior,
not an old migration count: it validates the single manual bootstrap exclusion,
full idempotent replay, RPC-only recipe writes, duplicate-line aggregation, and
idempotent stock consumption on retry.

`scripts/supabase-live-preflight.mjs` now verifies the **effective September
contract** (`p_appointment_id`, visit RPC, recipe tables, RPC-only write boundary,
duplicate-line aggregation and internal consumer revocation) rather than
allowing the older August checkout migration to stand in for the current
runtime contract.

The hosted LENA Beauty Demo (`tuzzvqsnbtzvkffmazyf`) has the matching four
visit/recipe migration versions applied. RLS, SECURITY DEFINER search paths,
grants/revokes, function signatures, constraints, indexes and the final recipe
consumption function were inspected directly on the hosted schema.
